...
startScreen.addEventListener('click', async () => {
  ...
  async function fetchDiscordActivity() {
    try {
      const res = await fetch(`https://api.lanyard.rest/v1/users/1214845468928901123`);
      ...
      if (!data.success) {
        if (data.error?.code === "user_not_monitored") {
          element.textContent = "Join Chibimaru server to show status";
          element.style.cursor = "pointer";
          element.onclick = () => window.open("https://discord.gg/chibimaru", "_blank");
          return;
        }
